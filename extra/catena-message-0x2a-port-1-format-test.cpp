...
Name:   catena-message-0x2a-port-1-format-test.cpp

Function:
    Generate test vectors for port 0x01 format 0x2a messages.
...
void encodeMeasurement(Buffer &buf, Measurements &m)
    ...
    if (m.CompostTemp.fValid)
        ...
        buf.push_back_be(encodeT(m.CompostTemp.v.t1));
        }
    ...
    if (m.CompostTemp.fValid)
        ...
        buf.push_back_be(encodeT(m.CompostTemp.v.t2));
        }
...
void logMeasurement(Measurements &m)
    ...
    if (m.Light.fValid)
        {
        std::cout << pad.get() << "Light " << (float) m.Light.v.lux;
        }

    if (m.CompostTemp.fValid)
        {
        std::cout << pad.get() << "CompostTemp " << m.CompostTemp.v.t1;
        }

    if (m.CompostTemp.fValid)
        {
        std::cout << pad.get() << "CompostTemp " << m.CompostTemp.v.t2;
        }
